...
module.exports = {
    loginAdmin: async (req, res) => {
        try {
            ...
            const deletedAdmin = await dba(sql, [emailorusername, emailorusername, password]);
            if (deletedAdmin.length) {
                return res.status(403).send({ message: 'Admin account is deleted, please contact Super Admin!' });
            }
            ...
            if (dataAdmin.length) {
                ...
                return res.status(500).send({ message: "The username or password you entered does not match our records. Please check and try again." });
            }
        } catch (error) {
            console.log(error);
