...
module.exports = {
  ProcessingProduct: async (req, res) => {
    try {
      ...
      sql = `select p.name as productName, od.product_id, od.qty, ifnull(stock,0) as stock, od.on_request from orders_detail od left join (select sum(qty) as stock,products_id from products_location 
      where readyToSend = 0 and warehouse_id = ?  group by products_id) as pl
      on od.product_id = pl.products_id 
  ...
  RequestStockToAnotherWarehouse: async (req, res) => {
    try {
      ...
      let sql = `select w.id as warehouse_id , w.location as warehouse, w.latitude, w.longitude
                ...
                where u.uid = ?`;
      const dataAdmin = await dba(sql, [uid]);
      // filter qtyReq yang bukan 0
      let newTransactionOrder = transactionOrder.filter(
        (val) => val.qtyReq > 0
      );
      newTransactionOrder.forEach(async (val) => {
        let dataInsert = {
  ...
  SendingItem: async (req, res) => {
    try {
      ...
      transactionDetail.forEach(async (val) => {
        ...
        sql = `update products_location set ? where orders_id = ? and warehouse_id = ? and products_id = ? and readyToSend = 1`;
        await dba(sql, [
          dataUpdate,
          row.orders_id,
          transaction[0].warehouse_id,
          val.product_id,
        ]);
      });
      let dataUpdate = {
